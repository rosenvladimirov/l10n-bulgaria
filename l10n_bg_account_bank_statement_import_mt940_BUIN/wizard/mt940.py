...
def handle_common_subfields(transaction, subfields):
    ...
    # Get counterpart from 30, 32 or 38 subfields:
    counterpart_fields = []
    for counterpart_field in ['30', '32', '38']:
...
class MT940Parser(MT940):
    ...
    def handle_tag_25(self, data):
        ...
        if data.find('BUIN') == -1:
            raise ValueError(
                'This is not Allianz Bank Bulgaria Bank Account %s' % data
                )
        self.account_number = data

    def handle_tag_28C(self, data):
        """Number of First Investment Bank bank statement."""
        self.current_statement['name'] = data.replace('.', '').strip()
        self.current_statement['name'] = self.mt940_type + " - " + self.current_statement['name'] + ' - ' + self.account_number 
