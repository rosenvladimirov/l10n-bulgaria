...
class MT940Parser(MT940):
    ...
    tag_61_regex = re.compile(
            ...
            r'(?P<sign>[CD])[NR](?P<amount>\d+,\d{2})(?P<type>.{1,10})'
        )

    def __init__(self):
        """Initialize parser - override at least header_regex."""
        super(MT940Parser, self).__init__()
        self.mt940_type = 'RZBB'
        self.header_lines = 0
        self.header_regex = r'^\s(.*)'
    ...
    def pre_process_data(self, data):
        ...
        holdline = ''
        data_ns = []
        #_logger.info("Split lines %s" % data.splitlines(True))
        for x in data.splitlines(True):
            if x.startswith(':NS:') or not x.startswith(':'):
                holdline += x.replace('\n', '')
            elif x.startswith(':20:') or x.startswith(':86:') or x.startswith(':60F:'):# reset hold line
                data_ns.append(holdline+'\n')
                data_ns.append(x)
                holdline = ''
            else:
                data_ns.append(x)
        data = ''.join(data_ns)
        #_logger.debug("Raifissen %s:%s" % (data, data.startswith('\n:20:GL')))
        if data.startswith('\n:20:GL'):
    ...
    def handle_tag_25(self, data):
        ...
        if data.find('RZBB') == -1:
            raise ValueError(
                'This is not Raifassen Bank Bank Account %s' % data
                )
        self.account_number = data
        _logger.info("ACCOUNT NUMBER %s" % self.account_number)

    def handle_tag_28C(self, data):
        """Number of Unicredit Bulbank bank statement."""
        self.current_statement['name'] = data.split(':NS:')[0].replace('.', '').strip()
