...
class BGVatViesReport(models.Model):
    ...
    tag_0501 = fields.Char(
        "[05-01/07-01] Section Code *ICA/Storage Mode*",
        readonly=True,
        help="Code of the section *ICA/Transfer of goods under the mode of storage of goods on demand* VIR",
    )
    tag_0502 = fields.Integer(
        "[05-02/07-02] Line number", readonly=True, help="Line number"
    )
    tag_0503 = fields.Char(
        "[05-03] VIN number",
        readonly=True,
        help="VIN number of the foreign partner, incl. the sign of the Member State",
    )
    tag_0504 = fields.Float(
        "[05-04] Total value of the tax base",
        readonly=True,
        help="Total value of the tax base for supplies of goods",
    )
    tag_0505 = fields.Float(
        "[05-05] Three-way operation",
        readonly=True,
        help="Supplies of goods as an intermediary in a tripartite operation",
    )
    tag_0506 = fields.Float(
        "[05-06] Delivered services under Art. 21",
        readonly=True,
    ...
    def _select_domain_type(self, form_id):
        if form_id.account_forms == "vat_form":
            domain_type = (
                """ANY({'out_invoice', 'out_refund', 'out_receipt'}::text[])"""
            )
        else:
            domain_type = "'entry'"
        return domain_type
