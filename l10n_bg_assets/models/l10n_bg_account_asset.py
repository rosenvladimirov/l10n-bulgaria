...
from odoo.tools import float_compare, start_of, end_of, date_range, add
...
class AccountAsset(models.Model):
    ...
    l10n_bg_depreciation_ids =  fields.One2many('bg.account.asset.depreciation.board',
                                                inverse_name='asset_id',
                                                string='BG law Depreciation')
    l10n_bg_freeze_period_ids = fields.One2many('bg.account.asset.freeze.periods',
                                                inverse_name='asset_id',
                                                string='Freeze periods')
    l10n_bg_method_percentage = fields.Float(string='Percentage Depreciation')
    l10n_bg_tax_model_id = fields.Many2one('account.asset', string='Tax Model',
                                           change_default=True, domain="[('company_id', '=', company_id)]")
    ...
    def _get_freeze_periods(self, bg_period_end_date):
        # Totally wrong logic of pause in future will replace with freeze table
        # This to be compatible with ee
        # date_before_pause = max(self.depreciation_move_ids, key=lambda
        #     x: x.date).date if self.depreciation_move_ids else self.asset_id.acquisition_date
        paused_prorata_date = datetime.combine(self.paused_prorata_date, datetime.min.time())
        end_paused_prorata_date = self.company_id.compute_fiscalyear_dates(paused_prorata_date).get('date_to')
        paused_prorata_dates = date_range(paused_prorata_date, end_paused_prorata_date)
        # _logger.info(f"Star/Stop Dates {end_paused_prorata_date}->{paused_prorata_date}")
        paused_mounts = relativedelta(end_paused_prorata_date - paused_prorata_date).months
    ...
    def _recompute_board(self, start_depreciation_date=False):
        self.ensure_one()
        depreciation_move_values = super()._recompute_board(start_depreciation_date=start_depreciation_date)
        ...
        sequence = 0
        l10n_bg_method_percentage = self.l10n_bg_method_percentage != 0.0 or 1.0
        ...
        bg_period_start_date = self.prorata_date == self.prorata_date.replace(day=1) and self.prorata_date or self.prorata_date + relativedelta(months=1)
        bg_period_start_date = bg_period_start_date.replace(day=1)
        bg_period_start_date = datetime.combine(bg_period_start_date, datetime.min.time())

        bg_period_end_date = (bg_period_start_date + relativedelta(months=1)) + relativedelta(days=-1) + relativedelta(months=months, years=years) + relativedelta(months=-1)
        ...
        paused_prorata_dates, paused_mounts, add_year, bg_period_end_date = self._get_freeze_periods(bg_period_end_date)

        l10n_bg_depreciation_ids = [Command.create({
            'sequence': sequence,
            'init_entry': True,
        ...
        for period in date_range(bg_period_start_date, bg_period_end_date):
            ...
            period = (period.replace(day=1) + relativedelta(months=1)) + relativedelta(days=-1)
            l10n_bg_depreciation_ids.append(Command.create({
                'sequence': sequence,
                'line_date': period,
                'company_id': self.env.company.id,
                'value_residual': residual_amount,
