...
class AccountAssetDepreciationBoard(models.Model):
    ...
    asset_id = fields.Many2one(
        "account.asset", string="Asset", index=True, ondelete="cascade"
    )
    ...
    method_percentage = fields.Float(
        string="Percentage Depreciation", related="asset_id.l10n_bg_method_percentage"
    )
    ...
    sleep_period_id = fields.Many2one(
        "bg.account.asset.freeze.periods", string="Sleeping period"
    )
