...
class BGAssetReportCustomHandler(models.AbstractModel):
    ...
    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals):
        ...
        if options['assets_groupby_tax_profile']:
            ...
            lines = report._regroup_lines_by_name_prefix(options, lines,
                                                         '_report_expand_unfoldable_line_assets_report_prefix_group', 0)
        ...
        for column_data in options['columns']:
            ...
            if column_data.get('figure_type') == 'monetary':
                total_columns.append({'name': report.format_value(col_value, self.env.company.currency_id,
    ...
    def _generate_report_lines_without_grouping(self, report, options, prefix_to_match=None, parent_id=None,
        ...
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            ...
            lines_query_results = self._query_lines(column_group_options, prefix_to_match=prefix_to_match,
    ...
    def _custom_options_initializer(self, report, options, previous_options=None):
        ...
        groupby_activated = (previous_options or {}).get('assets_groupby_tax_profile', True)
        ...
        options['unfold_all'] = (self._context.get('print_mode') and not options.get('unfolded_lines')) or (
                    report.filter_unfold_all and (previous_options or {}).get('unfold_all',
                                                                              not prefix_groups_threshold))
    ...
    def _query_lines(self, options, prefix_to_match=None, forced_model_id=None):
        ...
        """
        lines = []
        asset_lines = self._query_values(options, prefix_to_match=prefix_to_match, forced_model_id=forced_model_id)
        ...
        for al in parent_lines:
            ...
            if asset_method_percentage:
                asset_method_percentage = ('{:.2f} %').format(float(asset_method_percentage) * 100)
            else:
            ...
            asset_disposal_value = al['asset_disposal_value'] if al['asset_disposal_date'] and al[
                'asset_disposal_date'] <= fields.Date.to_date(options['date']['date_to']) else 0.0
            ...
            columns_by_expr_label = {
                "acquisition_date": al["asset_acquisition_date"] and format_date(self.env,
                                                                                 al["asset_acquisition_date"]) or "",
                # Characteristics
    ...
    def _group_by_tax_profile(self, report, lines, options):
        ...
        for line in lines:
            ...
            line_vals_per_model_id.setdefault(parent_model_id, {
                # We don't assign a name to the line yet, so that we can batch the browsing of account.account objects
                'id': report._build_line_id([(None, 'account.asset', parent_model_id)]),
        ...
        for model in models:
            ...
            model_line_vals['name'] = f"{model.name}: {model.l10n_bg_method_percentage*100:.2f}%"
            ...
            for model_sub_line in group_lines:
                ...
                for column_index in idx_monetary_columns:
                    group_totals[column_index] += model_sub_line['columns'][column_index].get('no_format', 0)
    ...
    def _query_values(self, options, prefix_to_match=None, forced_model_id=None):
        ...
        models_query = ''
        if forced_model_id:
            models_query = "AND account.id = %(forced_model_id)s"
        ...
        sql = f"""
             ...
             LEFT JOIN account_asset AS tax_model ON tax_model.l10n_bg_tax_model_id = asset.id
    ...
    def _report_expand_unfoldable_line_assets_report_prefix_group(self, line_dict_id, groupby, options, progress,
        ...
        lines, _totals_by_column_group = self._generate_report_lines_without_grouping(
            ...
            forced_model_id=self.env['account.report']._get_res_id_from_line_id(line_dict_id, 'account.asset'),
        )
