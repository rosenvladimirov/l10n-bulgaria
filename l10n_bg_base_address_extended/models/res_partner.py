...
class Partner(models.Model):
    ...
    def write(self, vals):
        ...
        if ('country_id' and 'city_id') in vals:
            country = self.env['res.country'].browse(vals.get('country_id', False) and vals['country_id'] or self.country_id.id)
            if country.enforce_cities:
                street = False
                res_street = {'name': self.street_name, 'country_id': self.country_id.id, 'city_id': self.city_id.id}
                if self.street_name and (self.street_number or self.street_number2) and not (self.street_building_number or self.street_floor_number or self.street_sector_number):
                    # name, number, number2
                    street = self.env['res.city.street'].search([('name', 'ilike', self.street_name),
                                                            ('number', 'ilike', self.street_number or ''),
                                                            ('number2', 'ilike', self.street_number2 or '')], limit=1)
                elif self.street_name and (self.street_building_number or self.street_floor_number or self.street_sector_number):
                    ...
                    street = self.env['res.city.street'].search([('sector_number', 'ilike', self.street_sector_number),
                                                            ...
                                                            ('number', 'ilike', self.street_number or ''),
                                                            ('building_number', 'ilike', self.street_building_number or ''),
                                                            ('floor_number', 'ilike', self.street_floor_number or ''),
