...
def post_init_hook(env):
    ...
    files = {
        ...
        "res.city.municipality": (
            2,
            os.path.join(module_path, "res.city.municipality.csv"),
        ),
        ...
        with open(file=file_name) as csv_file:
            ...
            for row in cn_codes:
                ...
                for fname_row, value in row.items():
                    ...
                    fname_check, lang_row = (
                        len(fname_split) > 1 and fname_split or fname_split + ["en_US"]
                    )
                    ...
                    if (
                        ...
                        for lang in filter(lambda r: r == lang_row, langs):
                            ...
                            if value_translated:
                                translation_importer.model_translations[mname][
                                    field.name
                                ][record_xml_id][lang] = value_translated
                            else:
                                translation_importer.model_translations[mname][
                                    field.name
                                ][record_xml_id][lang] = value
