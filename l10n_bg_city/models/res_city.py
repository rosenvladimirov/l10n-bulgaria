...
class City(models.Model):
    ...
    l10n_bg_type_settlement_id = fields.Many2one("res.city.types",
                                                 string="Type of settlement")
    l10n_bg_ecattu = fields.Char("UCATTU",
                                 help="The unified classifier "
                                      "of administrative-territorial and territorial units")
    l10n_bg_code = fields.Char(string='City Code',
                               help='The state code.')
    l10n_bg_city_hall_id = fields.Many2one('res.city',
                                           'City Hall',
                                           domain=lambda self: self._domain_l10n_bg_city_hall_id())
    l10n_bg_municipality_id = fields.Many2one('res.city', 'Municipality',
                                              domain=lambda self: self._domain_l10n_bg_city_hall_id())
    l10n_bg_has_tax_office = fields.Boolean("Has Tax Office")
    l10n_bg_structure_type = fields.Selection(string='Country Structure',
                                              selection=BG_CITY_STRUCTURE,
                                              default='normal')
