...
class AiEmbeddingProvider(models.AbstractModel):
    ...
    Configuration read from self.env.user (Claude Terminal tab):
    ...
    @api.model
    def embed(self, texts, user=None):
        ...
        if provider == "ollama":
            return self._embed_ollama(texts, user)
        if provider == "openai":
            return self._embed_openai(texts, user)
        if provider == "voyage":
            return self._embed_voyage(texts, user)
    ...
    @api.model
    def vector_size(self, user=None):
        ...
        call is used if the model is unknown — cached on the user's
        config is NOT attempted here to keep the call stateless.
        """
        user = user or self.env.user
        provider = user.claude_embedding_provider or "ollama"
        model = (user.claude_ollama_model or "nomic-embed-text").lower()
        ...
        vec = self.embed(["__probe__"], user=user)
    ...
    def _embed_ollama(self, texts, user):
        url = (user.claude_ollama_url or "").rstrip("/")
        model = user.claude_ollama_model or "nomic-embed-text"
        if not url:
    ...
    def _embed_openai(self, texts, user):
        api_key = user.claude_embedding_api_key or ""
        if not api_key:
            raise UserError("OpenAI API key is not configured.")
        model = user.claude_ollama_model or "text-embedding-3-small"
    ...
    def _embed_voyage(self, texts, user):
        api_key = user.claude_embedding_api_key or ""
        if not api_key:
            raise UserError("Voyage API key is not configured.")
        model = user.claude_ollama_model or "voyage-3"
        if model.startswith("nomic") or model.startswith("mxbai"):
            model = "voyage-3"
        payload = json.dumps({
            "model": model,
