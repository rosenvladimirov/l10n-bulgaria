...
class BulgarianReportCustomer(models.AbstractModel):
    ...
    def _custom_options_initializer(self, report, options, previous_options=None):
        ...
        if options.get('_running_export_test'):
            ...
            options['bg_vat_book_tax_type_selected'] = previous_options.get('bg_vat_book_tax_type_selected', 'all')
            
        tax_types = self._vat_book_get_selected_tax_types(options)
        
        columns_to_remove = []
        if not self.env['account.tax'].search([('type_tax_use', 'in', tax_types), ('tax_group_id.l10n_bg_vat_code', '=', '1')]):
            columns_to_remove.append('vat_9')
        if not self.env['account.tax'].search([('type_tax_use', 'in', tax_types), ('tax_group_id.l10n_bg_vat_code', '=', '2')]):
            columns_to_remove.append('vat_20')
        if not self.env['account.tax'].search([('type_tax_use', 'in', tax_types), ('tax_group_id.l10n_bg_vat_code', '=', '0')]):
    ...
    def _build_query(self, report, options, column_group_key):
        ...
        return self.env['account.bg.vat.line']._bg_vat_line_build_query(tables, where_clause, where_params, column_group_key, tax_types)
    ...
    def vat_book_export_files_to_zip(self, options):
        ...
        # Build file name
        export_file_name = {'sale': 'VAT_Book_Sales', 'purchase': 'VAT_Book_Purchase', 'declar': 'VAT_Book_Declar'}.get(tax_type, 'VAT_Book')
        ...
        with zipfile.ZipFile(stream, 'w', compression=zipfile.ZIP_DEFLATED) as zf:
            vouchers_data, aliquots_data, declaration, purchase = self._vat_book_get_txt_files(options, tax_type)
    ...
    def _vat_book_get_txt_files(self, options, tax_type):
        ...
        vouchers_data = '\r\n'.join(self._vat_book_get_REGINFO_SALES(options, tax_type, invoices)).encode('cp1251', 'ignore')
        declaration = '\r\n'.join(self._vat_book_get_REGINFO_DECLARATION(options, tax_type, invoices)).encode('cp1251', 'ignore')
        purchase = '\r\n'.join(self._vat_book_get_REGINFO_PURCHASE(options, tax_type, invoices)).encode('cp1251', 'ignore')
    ...
    def _vat_book_get_REGINFO_SALES(self, options, tax_type, invoices): 
        ...
        for inv in invoices:
            ...
            amount_sales = self._calculate_amount_for_different_sales_taxes(inv, options)
    ...
    def _vat_book_get_REGINFO_PURCHASE(self, options, tax_type, invoices): 
        ...
        for inv in invoices:
            ...
            amount_purchase = self._calculate_amount_for_different_purchases_taxes(inv, options)
    ...
    def _vat_book_get_REGINFO_DECLARATION(self, options, tax_type, invoices): 
        ...
        amounts_purchase = self._calculate_amount_for_different_purchases_taxes(invoices, options)
    ...
    def _calculate_sales_invoice_totals(self, options, invoices):
        ...
        invoices = self._filter_invoices_by_date_range(options, invoices, 'invoice_date')
        invoices = self._filter_invoices_type(options, invoices, ['out_invoice', 'out_refund'])
        
        # SALES TOTAL AMOUNT and SALES VAT TOTAL AMOUNT
        return {
            'sales_total': sum(abs(inv.price_subtotal) for inv in invoices.invoice_line_ids),
    ...
    def _calculate_amount_for_different_purchases_taxes(self, invoices, options):
        ...
        invoices = self._filter_invoices_type(options, invoices, ['in_invoice', 'in_refund'])
        ...
        for tag, id in tax_tag_ids.items():
            total_amounts[tag] = sum([abs(line.balance) for line in invoices.line_ids if line.tax_tag_ids.id == id])
            
        for tag, id in tax_tag_ids_neg.items():
            total_amounts[tag] = sum([abs(line.balance) for line in invoices.line_ids if line.tax_tag_ids.id == id])
            
        total_amounts['amount_tag_31'] = total_amounts['amount_tag_31'] - total_amounts['amount_tag_31_neg']
        total_amounts['amount_tag_32'] = total_amounts['amount_tag_32'] - total_amounts['amount_tag_32_neg']
        total_amounts['amount_tag_41'] = total_amounts['amount_tag_41'] - total_amounts['amount_tag_41_neg']
        total_amounts['amount_tag_42'] = total_amounts['amount_tag_42'] - total_amounts['amount_tag_42_neg']
        total_amounts['amount_tag_30'] = total_amounts['amount_tag_30'] - total_amounts['amount_tag_30_neg']

        total_amounts['vat_tax_40'] = sum([inv.amount_tax for inv in invoices if inv.move_type in ['in_invoice', 'in_refund']])
    ...
    def _calculate_amount_for_different_sales_taxes(self, invoices, options):
        ...
        for tag, ids in tag_tax_ids.items():
            total_amounts[tag] = sum([abs(line.balance) for line in invoices.line_ids if line.tax_tag_ids.id in ids])
    ...
    def _get_document_number(self, invoice):
        ...
        return invoice.ref if invoice.move_type in ['in_invoice', 'in_refund'] else invoice.name
    
    def _get_document_type(self, inv):
        # Returns invoice type
        return {'in_invoice': '01', 'out_invoice': '01', 'out_refund': '03', 'in_refund': '03'}.get(inv.move_type, None)
    ...
    def _filter_invoices_type(self, options, invoices, move_types):
        ...
        filtered_invoices = invoices.filtered(
            lambda inv: inv.move_type in move_types
        )
    ...
    def _filter_invoices_by_date_range(self, options, invoices, custom_date_attribute):
        ...
        filtered_invoices = invoices.filtered(
            lambda inv: date_from_date <= getattr(inv, custom_date_attribute) <= date_to_date
        )
    ...
    def _base_number_formatting(self, value, max_length):
        ...
        formatted_value = "{:.2f}".format(value).rjust(max_length)
