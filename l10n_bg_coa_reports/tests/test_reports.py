...
class TestBulgarianReportCustomer(TransactionCase):
    ...
    def test_filter_invoices_type_purchases(self):
        ...
        filtered_invoices = self.env[
            "l10n_bg_tax_report_handler"
        ]._filter_invoices_type_purchases(test_options, test_invoices)
    ...
    def test_filter_invoices_type_sales(self):
        ...
        filtered_invoices = self.env[
            "l10n_bg_tax_report_handler"
        ]._filter_invoices_type_sales(test_options, test_invoices)
    ...
    def test_filter_invoices_by_date_range(self):
        ...
        filtered_invoices = self._filter_invoices_by_date_range(
            options, self.env["account.move"]
        )
        ...
        self.assertIn(
            invoice1,
            filtered_invoices,
            "Invoice 1 should be included in the filtered invoices",
        )
        self.assertIn(
            invoice2,
            filtered_invoices,
            "Invoice 2 should be included in the filtered invoices",
        )
    ...
    def test_filter_invoices_by_date_range_purchases(self):
        ...
        # Call the method to be tested
        filtered_invoices = self._filter_invoices_by_date_range_purchases(
            options, self.env["account.move"]
        )
        ...
        self.assertIn(
            vendor_bill_one,
            filtered_invoices,
            "Vendor Bill 1 should be included in the filtered invoices",
        )
        self.assertIn(
            vendor_bill_two,
            filtered_invoices,
            "Vendor Bill 2 should be included in the filtered invoices",
        )
