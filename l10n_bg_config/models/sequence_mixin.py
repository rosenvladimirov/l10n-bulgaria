...
class SequenceMixin(models.AbstractModel):
    ...
    def _get_sequence_fields(self, sequence):
        return {
            self._sequence_field: sequence
            }
        ...
        while True:
            ...
            try:
                with self.env.cr.savepoint(flush=False), mute_logger('odoo.sql_db'):
                    sequence_fields = self._get_sequence_fields(sequence)
                    self.update(sequence_fields)
                    self.flush_recordset(list(sequence_fields.keys()))
                    break
