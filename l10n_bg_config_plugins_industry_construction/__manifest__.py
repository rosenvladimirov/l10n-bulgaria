...
{
    "name": "Bulgaria - Industry Pack: Construction",
    "summary": """
        Industry configuration pack for КИД sector F (Construction):
        ...
        positions / analytic).""",
    "description": """
Bulgaria - Industry Pack: Construction (КИД sector F)
==================================================

Configuration pack for КИД sector F — construction (work-in-progress on construction contracts under СС 11).

What it does
------------
* On install, adds КИД section **F** to every Bulgarian company's
  *Active КИД sectors* (``res.company.l10n_bg_kid_ids``) and reloads
  the chart of accounts, so the "one chart, many КИД" install filter
  (``l10n_bg_config``) lets sector F's sector-specific accounts
  through.
* Is the designated home for sector F's accounting content shipped
