...
from odoo.addons.l10n_bg_erp_net_base.models.erp_net_fp_exceptions import (
    FiscalPrinterError,
...
class FiscalPrinterDevice(models.Model):
    """Генеричен транспорт до хардуерния proxy Odoo.ErpNet.FP.

    Тук е САМО транспортният слой: host/connection_mode + HTTP/bus
    заявки към проксито. Фискалната логика (Z/X/PLU/отчети) се
    закача от `l10n_bg_erp_net_fp` през `_inherit`.
    Името на модела остава `fiscal.printer.device` за data-съвместимост.
    """

    _name = 'fiscal.printer.device'
    _description = 'ErpNet.FP proxy device'
    ...
    company_id = fields.Many2one(
        ...
        help='Tenant that owns this proxy device.',
    )
    ...
    def _proxy_or_direct_action(self, endpoint, success_title, success_msg,
        ...
        try:
            # timeout_ms (ms, browser бюджет) → timeout (сек) за
            # server-side заявката; иначе бавните операции (X/Z ~90s)
            # падат на device default-а (30s).
            self._make_request(
                method, endpoint,
    ...
    def open_cash_drawer(self):
        ...
        Генерична команда към устройството (без фискална семантика).
        """
        self.ensure_one()
        if self.connection_mode == "proxy":
    ...
    def list_proxy_printers(self, host=None, ssl_verify=False, timeout=8):
        """Изброява принтерите на проксито (за printer_id discovery widget)."""
        if not host:
