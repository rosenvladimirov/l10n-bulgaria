...
class L10nBGHrVersionAmendment(models.Model):
    ...
    def _l10n_bg_validate_calendar(self):
        ...
        # 🚨 DEF-175 (Пламена, 11.09.2026): дотук И ДВЕТЕ страни се извеждаха
        # от присъствията, а ядрото дефинира `_get_hours_per_day()` като
        # `_get_hours_per_week() / _get_days_per_week()`. При петдневна седмица
        # сравнението беше ТАВТОЛОГИЯ — разликата излиза нула за всякакви
        # присъствия и гардът не можеше да гръмне. Капанът минаваше през него:
        # календар 21 с присъствия 08:00–17:00 и записано `hours_per_day` 4,00
        # дава 45,00 срещу 9,00 × 5 = 45,00.
        #
        # Числото, по което ФИШЪТ дели, е `calendar.hours_per_day` — виж
        # `_l10n_bg_hours_per_day_for` в `hr_payslip`. Затова едната страна е
        # вече то, а другата остава изведена от присъствията: сравняваме
        # твърдението със свидетелството, не свидетелството със себе си.
        sedmichni = calendar._get_hours_per_week()
        # Празно `hours_per_day` НЕ е противоречие: фишът тогава пада на
        # изведеното от присъствията и двете числа са съгласувани по
        # конструкция. Противоречие има само когато някой е записал свое число.
        dnevni = calendar.hours_per_day or calendar._get_hours_per_day()
        ...
        #
        # ⚠️ Допреди DEF-175 тази константа НЕ значеше нищо при петдневен
        # календар: и двете страни идваха от присъствията и се съкращаваха.
        # Сега тя има ефект. Поведението за шестдневен календар не се променя —
        # той се отказваше и преди, — но вече по разбираема причина. Ако
        # такава норма трябва да минава (чл. 1, ал. 4 НЕВДПОВ мери по часове,
        # не по дни), делителят става `calendar._get_days_per_week()`; това е
        # същият въпрос като по т. 5 на ревизията и чака решение, не догадка.
    ...
    def _apply_version_changes(self):
        ...
        #
        # ⚖️ Записва се СРОКЪТ, не прекратяването (ADR l10n-bg-ds-amendment/0017). Чл. 68 КТ дава
