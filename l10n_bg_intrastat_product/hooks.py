...
def pre_init_hook(cr):
    ...
    if not os.path.isfile(module_path + "2023_bg_intrastat_codes.csv"):
        shutil.copy2(
            source_path + "2023_bg_intrastat_codes.csv",
            module_path + "2023_bg_intrastat_codes.csv",
        )
...
def post_init_hook(cr, registry):
    ...
    with open(
        file=os.path.join(module_path, "intrastat.transaction.bg.csv")
    ) as csv_file:
    ...
    with open(file=os.path.join(module_path, "intrastat.region.bg.csv")) as csv_file:
        cn_codes = csv.DictReader(csv_file, delimiter=",")
        for row in cn_codes:
            country_id = env.ref("base.bg")
            state_code_id = env["res.country.state"].search(
                [("country_id", "=", country_id.id), ("code", "=", row["code"].strip())]
            )
            res_id = env.ref(row["id"])
            if res_id:
            ...
            if state_code_id:
                for city in env["res.city"].search(
                    [
    ...
    with open(
        file=os.path.join(module_path, "intrastat.transport_mode.bg.csv")
    ) as csv_file:
        cn_codes = csv.DictReader(csv_file, delimiter=";")
        for row in cn_codes:
            res_id = env.ref(row["id"])
            if res_id:
                res_id = env["intrastat.transport_mode"].search(
                    [("id", "=", res_id.id)]
                )
                if res_id:
                    res_id.with_context(lang="bg_BG").write(
