...
class IntrastatProductDeclaration(models.Model):
    ...
    def _get_origin_transport(self, inv_line, notedict):
        ...
        origin_transport = invoice.origin_transport_id \
    ...
    def _update_computation_line_vals(self, inv_line, line_vals, notedict):
        invoice = inv_line.move_id
        line_vals.update({
            'origin_transport_id': self._get_origin_transport(inv_line, notedict).id,
    ...
    def _generate_xml(self):
        ...
        envelope_address_phone_number = etree.SubElement(envelope_address, 'phoneNumber')
        envelope_address_phone_number.text = envelope_contact.phone and envelope_contact.phone or ''

        envelope_address_mobile = etree.SubElement(envelope_address, 'mobilePhoneNumber')
        envelope_address_mobile.text = envelope_contact.mobile and envelope_contact.mobile or ''
        envelope_address_mail = etree.SubElement(envelope_address, 'e-mail')
        envelope_address_mail.text = envelope_contact.email and envelope_contact.email or ''
        ...
        assert self.declaration_type in ('arrivals', 'dispatches'), \
        ...
        total_statistical_value.text = str(int(sum([round(x.amount_company_currency, 0) for x in
        ...
        for pline in self.declaration_line_ids:
            ...
            weight.text = "{0:.3f}".format(pline.weight)
            ...
            if not pline.suppl_unit_qty:
                quantity_in_SU.text = "{0:.3f}".format(0.0)
            else:
                quantity_in_SU.text = "{0:.3f}".format(pline.suppl_unit_qty)
