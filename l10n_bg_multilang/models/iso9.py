...
iso9, _iso9 = {}, cyrillic
#_logger.info("Char %s" % _iso9)
for cyrmaj, cyrmin, latmaj1, latmaj2, latmaj3, latmin1, latmin2, latmin3 in _iso9:
    #_logger.info("Unpack %s,%s,%s,%s,%s,%s,%s,%s" % (cyrmaj, cyrmin, latmaj1, latmaj2, latmaj3, latmin1, latmin2, latmin3))
    iso9[cyrmaj] = ''.join([latmaj1, latmaj2, latmaj3])
    iso9[cyrmin] = ''.join([latmin1, latmin2, latmin3])

class Transliterate(models.AbstractModel):
    _name = "l10n_bg_multilang.transliterate"

    def transliterate(self, source):
        result = []
        if not source:
            return ''
        l = len(source)
