...
def get_l10n_bg_applicability(res):
    return [
        ("declaration", _lt("Declaration")),
        ("purchase", _lt("Purchase report")),
        ("sale", _lt("Sale report")),
        ("vies", _lt("VIES Report")),
    ]
...
def get_doc_type():
    return [
        ...
        ("02", _lt("Debit note")),
        ("03", _lt("Credit note")),
        ("04", _lt("Storeable goods sent to EU")),
        ...
        ("05", _lt("Storeable goods receive from EU")),
        ("07", _lt("Customs declarations")),
        # Customs declaration/customs document certifying completion of customs formalities
        ("09", _lt("Protocols or other")),
        ("11", _lt("Invoice - cash reporting")),
        ("12", _lt("Debit notice - cash reporting")),
        ("13", _lt("Credit notice - cash statement")),
        ("50", _lt("Protocols fuel supplies")),
        ("81", _lt("Sales report - tickets")),
        (
            "82",
            _lt("Special tax order"),
        ),  # Report on the sales made under a special tax order
        ("83", _lt("sales of bread")),  # Report on the sales of bread
        ("84", _lt("Sales of flour")),  # Report on the sales of flour
        (
            "23",
            _lt("Credit note art. 126b"),
        ),  # Credit notification under Art. 126b, para. 1 of VAT
        (
            "29",
            _lt("Protocol under Art. 126b"),
        ),  # Protocol under Art. 126b, para. 2 and 7 of VAT
        (
            "91",
            _lt("Protocol under Art. 151c"),
        ),  # Protocol for the required tax under Art. 151c, para. 3 of the law
        ("92", _lt("Protocol under Art. 151g")),
        # Protocol on the tax credit under Art. 151g,
        # para. 8 of the law or a report under Art. 104g, para. 14
        ("93", _lt("Protocol under Art. 151c")),
        ...
        ("94", _lt("Protocol under Art. 151c, para. 7")),
        # Protocol for the required tax under Art. 151c,
        # para. 7 of the law with a delivery recipient, a person who applies the special regime
        ("95", _lt("Protocol for free provision of foodstuffs")),
...
def get_type_vat():
    return [
        ("standard", _lt("Accounting document")),
        ("117_protocol", _lt("Art. 117 - Protocols")),
        ("119_report", _lt("Art. 119 - Report for sales")),
        # ('120_sales_report', _lt('Art. 119 - Report for sales-special rules')),
        # ('120_purchase_report', _lt('Art. 119 - Report for purchase-special rules')),
        ("in_customs", _lt("Import Customs declaration")),
        ("out_customs", _lt("Export Customs declaration")),
        ("dropship", _lt("Dropship/Try party deal")),
    ]


def get_delivery_type():
    return [
        ("01", _lt("Delivery under Part I of Annex 2 of VAT")),
        ("02", _lt("Delivery under Part II of Annex 2 of VAT")),
        ("03", _lt("Import under Annex 3 of VAT")),
        ("07", _lt("Supply, import or IC acquisition of bread")),
        ("08", _lt("Supply, import or IC acquisition of flour")),
        (
            "51",
            _lt(
        ...
        (
            "54",
            _lt("Marriage/absence/destruction of goods under Art. 15a, para. 10 of VAT"),
        ),
        (
            "58",
            _lt(
