...
class AccountBGCalcPartnerLine(models.Model):
    """Base model for new Bulgarian Parner traial balance reports."""
    ...
    SELECT
        ...
        period,
        account_type,
        SUM(CASE WHEN balance_type = 'initial' AND debit > credit THEN debit - credit ELSE 0 END) as initial_balance_debit,
        SUM(CASE WHEN balance_type = 'initial' AND credit > debit THEN credit - debit ELSE 0 END) as initial_balance_credit,
        ...
        SUM(CASE WHEN balance_type = 'final' AND debit > credit THEN debit - credit ELSE 0 END) as end_balance_debit,
        SUM(CASE WHEN balance_type = 'final' AND credit > debit THEN credit - debit ELSE 0 END) as end_balance_credit
    FROM account_data
