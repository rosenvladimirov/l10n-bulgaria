...
class AccountBGCalcPurchasesLine(models.Model):
    ...
    SUM(CASE WHEN am.state = 'cancel' THEN 0.00 WHEN aat.tag_name = 30 AND aat.negate THEN ABS(aml.balance)*-1
        WHEN aat.tag_name = 30 AND NOT aat.negate THEN ABS(aml.balance)
        ELSE 0.00 END) AS account_tag_30,
    SUM(CASE WHEN am.state = 'cancel' THEN 0.00 WHEN aat.tag_name = 31 AND aat.negate THEN ABS(aml.balance)*-1
        WHEN aat.tag_name = 31 AND NOT aat.negate THEN ABS(aml.balance)
        ELSE 0.00 END) AS account_tag_31,
    SUM(CASE WHEN am.state = 'cancel' THEN 0.00 WHEN aat.tag_name = 41 AND aat.negate THEN ABS(aml.balance)*-1
        WHEN aat.tag_name = 41 AND NOT aat.negate THEN ABS(aml.balance)
        ELSE 0.00 END) AS account_tag_41,
    SUM(CASE WHEN am.state = 'cancel' THEN 0.00 WHEN aat.tag_name = 32 AND aat.negate THEN ABS(aml.balance)*-1
        WHEN aat.tag_name = 32 AND NOT aat.negate THEN ABS(aml.balance)
        ELSE 0.00 END) AS account_tag_32,
    SUM(CASE WHEN am.state = 'cancel' THEN 0.00 WHEN aat.tag_name = 42 AND aat.negate THEN ABS(aml.balance)*-1
        WHEN aat.tag_name = 42 AND NOT aat.negate THEN ABS(aml.balance)
        ELSE 0.00 END) AS account_tag_42,
    SUM(CASE WHEN am.state = 'cancel' THEN 0.00 WHEN aat.tag_name = 43 AND aat.negate THEN ABS(aml.balance)*-1
        WHEN aat.tag_name = 43 AND NOT aat.negate THEN ABS(aml.balance)
        ELSE 0.00 END) AS account_tag_43,
    SUM(CASE WHEN am.state = 'cancel' THEN 0.00 WHEN aat.tag_name = 44 AND aat.negate THEN ABS(aml.balance)*-1
