...
class StockMove(models.Model):
    ...
    def _get_account_move_line_vals(self):
        ...
        # локация) се осчетоводяват по стандартния location-based механизъм —
        # през сметката на производствената локация (Cost of Production), а НЕ
        # през продажбената category output/input сметка (напр. 709.200
        # 'отчетна стойност на продадени материали', която е за реализация,
        # не за влагане). Така вложените в производство материали отиват в
        # производствената сметка и се позволява надграждане от
        # l10n_bg_mrp_account (транзит 601 → 611).
        if (
