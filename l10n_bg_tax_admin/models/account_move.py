...
class AccountMove(models.Model):
    ...
    l10n_bg_private_vat_invoice_ids = fields.Many2many(
        "account.move",
    ...
    l10n_bg_customs_invoice_id = fields.Many2one(
        "account.move",
        string="Base invoice",
        check_company=True,
        copy=False,
    ...
    l10n_bg_customs_date_custom_id = fields.Date(
        "Customs date", related="l10n_bg_customs_invoice_id.l10n_bg_customs_date"
    )
    l10n_bg_name_custom_id = fields.Char(
        "Customs number", related="l10n_bg_customs_invoice_id.l10n_bg_name"
    )
    ...
    # === Partner fields === #
    l10n_bg_customs_partner_id = fields.Many2one(
        "res.partner",
    ...
    def _default_l10n_bg_currency_rate(self):
        ...
        def get_rate(from_currency, to_currency, company, date):
            return (
                ...
                .with_context(dict(self._context, statistic_rate=True))
    ...
    def _get_invoice_reference_bg_invoice(self):
        ...
        if self.company_id.vat.lower().lstrip().startswith("bg"):
            ...
            base_name = (
                [decade] + ["0"] * (10 - len(base_name) - len(decade)) + base_name
            )
            name = "".join(base_name)
        else:
            name = self.name
    ...
    def _get_invoice_reference_bg_partner(self):
        ...
        if self.partner_id.l10n_bg_uic != "999999999999999":
            ...
            name = (self.partner_id.l10n_bg_uic or self.partner_id.vat).zfill(
                21
            ) + "".join([i for i in self.ref if i.isdigit()]).zfill(10)
        return name
    ...
    def _compute_tax_totals_signed(self):
        for move in self:
            if move.is_invoice(include_receipts=True):
                ...
                # _logger.info(f"base_lines: {base_lines}")
        ...
        for move in to_post:
            ...
            if map_id.new_account_entry:
                if map_id and not map_id.position_dest_id:
                    raise UserError(
                        _(
                            "Missing configuration for replacing fiscal position for export/import customs or deal by art. 82"
                        )
                    )
                ...
                if new_entry_id.l10n_bg_type_vat == "in_customs":
                    ...
                    customs_id = self.env["account.move.bg.customs"].create(
                        self.env["account.move.bg.customs"]._customs_vals(new_entry_id)
                    )
                    new_entry_id.write(
                        {
                            ...
                            "invoice_line_ids": customs_id._customs_aml(
                                move, new_entry_id, map_id
                            ),
                    ...
                    new_entry_id.l10n_bg_type_vat == "117_protocol"
                    and move.is_purchase_document()
                ):
                    l10n_bg_private_vat_id = self.env[
                        "account.move.bg.protocol"
            ...
            if (
                ...
                protocol_id = self.env["account.move.bg.protocol"].create(
                    self.env["account.move.bg.protocol"]._protocol_vals(move)
                )
                move.l10n_bg_protocol_invoice_id = protocol_id.id
                protocol_id._compute_protocol_name()
            if (
                move.is_sale_document(True)
                and move.l10n_bg_type_vat == "119_report"
                and not move.l10n_bg_report_sale_id
            ):
                report_sale_id = self.env["account.move.bg.report.sale"].create(
                    self.env["account.move.bg.report.sale"]._report_vals(move)
                )
    ...
    def _onchange_l10n_bg_customs_invoice_ids(self):
        if self.l10n_bg_customs_invoice_ids - self.l10n_bg_customs_invoice_id:
            map_id = self.l10n_bg_customs_invoice_id.fiscal_position_id.map_type(
                self.l10n_bg_customs_invoice_id
            )
            for invoice_id in (
                self.l10n_bg_customs_invoice_ids - self.l10n_bg_customs_invoice_id
            ):
                if self.invoice_line_ids.filtered(
                    lambda r: r.l10n_bg_customs_invoice_id == invoice_id
                ):
                    continue
                self.invoice_line_ids = self.l10n_bg_customs_id._customs_aml(
                    invoice_id, self, map_id
                )
            for line in self.line_ids:
                if (
                    line.l10n_bg_customs_invoice_id.id
                    not in self.l10n_bg_customs_invoice_ids.ids
                ):
                    line.unlink()
    ...
    def get_vendor_customs(self, include_receipts=False):
        return self.get_vendor_invoice(include_receipts=include_receipts), [
            "in_customs"
        ]
    ...
    def is_vendor_customs(self, include_receipts=False):
        move_type, l10n_bg_type_vat = self.get_vendor_customs(
    ...
    def get_customer_customs(self, include_receipts=False):
        return self.get_customer_invoice(include_receipts=include_receipts), [
            "out_customs"
        ]
    ...
    def is_customer_customs(self, include_receipts=False):
        move_type, l10n_bg_type_vat = self.get_customer_customs(
            include_receipts=include_receipts
        )
        if self.move_type in move_type and self.l10n_bg_type_vat in l10n_bg_type_vat:
            return True
