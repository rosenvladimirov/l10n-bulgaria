...
class AccountChartTemplate(models.Model):
    ...
    @api.model
    def _create_liquidity_journal_suspense_account(self, company, code_digits, grouping='[]'):
        return self.env['account.account'].create({
            'name': _("Bank Suspense Account"),
            'code': self.env['account.account']._search_new_account_code(company,
                                                                         code_digits,
...
FROM ir_model_data
    WHERE model = '{type_account.position_dest_id._name}'
        ...
        AND module = 'l10n_bg'""")
                ...
                if position_fiscal:
                    fiscal_position_id = self.env.ref(f"l10n_bg.{company_id.id}_{position_fiscal[0]}",
                                                      raise_if_not_found=False)
            type_invoice_ref.append((type_account, {
    ...
    def _get_templates(self, position, fp, **kwargs):
        ...
        for key, template_ref in kwargs.items():
            if key == 'tax_template_ref':
                ...
                for tax in position.tax_ids:
                    tax_template_vals.append((tax, {
                        ...
                        'tax_dest_id': tax.tax_dest_id and template_ref[tax.tax_dest_id].id or False,
                        'position_id': fp.id,
                    }))
            ...
            if key == 'type_invoice_ref':
                for type_account in position.type_ids:
                    template_vals['account.fiscal.position.type'].append((type_account, {
                        'position_id': fp.id,
    ...
    def generate_fiscal_position(self, tax_template_ref, acc_template_ref, company_id):
        ...
        fps = self._create_records_with_xmlid('account.fiscal.position', template_vals, company_id)
    ...
    def _load(self, company):
        ...
        if existing_accounts:
            ...
            models_to_delete = ['account.reconcile.model', 'account.fiscal.position', 'account.move.line',
                                'account.move', 'account.journal', 'account.tax', 'account.group']
        ...
        if not company.account_journal_suspense_account_id:
            company.account_journal_suspense_account_id = \
                self._create_liquidity_journal_suspense_account(company, self.code_digits, self.grouping)
        ...
        if not company.account_journal_payment_debit_account_id:
            company.account_journal_payment_debit_account_id = self.env['account.account'].create({
                ...
                'code': self.env['account.account']._search_new_account_code(company, self.code_digits,
                                                                             company.bank_account_code_prefix or '',
                                                                             self.grouping),
                'reconcile': True,
                'account_type': 'asset_current',
