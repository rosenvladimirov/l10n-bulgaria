...
class AccountMoveBgPrivate(models.Model):
    ...
    # COMPUTE METHODS
    # -------------------------------------------------------------------------
    @api.depends('l10n_bg_private_move_id.posted_before', 'l10n_bg_private_move_id.state',
                 'l10n_bg_private_move_id.l10n_bg_date')
    ...
    @api.depends('l10n_bg_private_move_id.l10n_bg_date', 'move_type', 'l10n_bg_private_name', 'posted_before',
                 'sequence_number', 'sequence_prefix', 'state')
    def _compute_l10n_bg_private_name_placeholder(self):
        for record in self:
            if (
                not record.l10n_bg_private_name or record.l10n_bg_private_name == '/') and not record._get_last_sequence():
    ...
    def _compute_l10n_bg_tax_totals(self):
        for private in self:
            ...
            if move.is_invoice(include_receipts=True):
                ...
                private.l10n_bg_tax_totals = None
    ...
    def _set_next_made_sequence_gap(self, made_gap: bool):
        ...
        for prefix, privates in named.grouped(lambda move: move.sequence_prefix).items():
            # Fixed search to use correct model
            next_privates += self.env['account.move.bg.private'].sudo().search([
                ('sequence_prefix', '=', prefix),
                ('sequence_number', 'in', [move.sequence_number + 1 for move in privates]),
            ])
    ...
    def _get_view(self, view_id=None, view_type='form', **options):
        ...
        if view_type == 'form':
            if name_node := arch.xpath(
                """//field[@name="l10n_bg_private_name"][@invisible="l10n_bg_private_name == '/' and not posted_before and not quick_edit_mode"]"""):
                name_node[0].set('invisible',
                                 "not (l10n_bg_private_name or l10n_bg_private_name_placeholder or quick_edit_mode)")
            if draft_node := arch.xpath(
                """//span[@invisible="l10n_bg_private_name == '/' and not posted_before and not quick_edit_mode"]"""):
                draft_node[0].set('invisible',
                                  "l10n_bg_private_name or l10n_bg_private_name_placeholder or quick_edit_mode")
        return arch, view
