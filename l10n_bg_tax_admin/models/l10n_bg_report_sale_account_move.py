...
class AccountMoveBgReportSale(models.Model):
    ...
    report_name = fields.Char(
        ...
        compute='_compute_report_name', inverse='_inverse_report_name', readonly=False, store=True,
        copy=False,
        tracking=True,
        index='trigram',
    )
    ...
    @api.depends('move_id.posted_before', 'move_id.state', 'move_id.journal_id', 'move_id.date', 'report_name')
        ...
        for report in self:
            ...
            if move_has_name or report.move_id.state != 'posted':
                if not report.posted_before and not report._sequence_matches_date():
                    ...
                    if move_has_name and report.posted_before or not move_has_name and report._get_last_sequence(lock=False):
                        continue
            if report.date and (not move_has_name or not report._sequence_matches_date()):
                report._set_next_sequence()

        self.filtered(lambda m: not m.report_name and not report.quick_edit_mode).report_name = '/'
    ...
    def _get_last_sequence_domain(self, relaxed=False):
        ...
        if not relaxed:
            domain = [('id', '!=', self.id or self._origin.id), ('report_name', 'not in', ('/', '', False))]
            reference_move_name = self.search(
                domain + [('report_date', '<=', self.report_date)],
                order='report_date desc',
                limit=1).report_name
            if not reference_move_name:
                reference_move_name = self.search(domain, order='report_date asc', limit=1).report_name
