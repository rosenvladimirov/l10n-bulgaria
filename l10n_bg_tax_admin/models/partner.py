...
class AccountFiscalPosition(models.Model):
    ...
    # Changed from Binary to Json for better compatibility with Odoo 17
    tax_action_map = fields.Json(compute='_compute_tax_action_map')
        ...
        for position in self:
            for tl in position.tax_action_map_ids:
                tax_action_map[f"{tl.move_type}-{position.id}"].update({
                    ...
                    'position_dest_id': tl.position_dest_id.id if tl.position_dest_id else False,
                    'account_id': tl.account_id.id if tl.account_id else False,
