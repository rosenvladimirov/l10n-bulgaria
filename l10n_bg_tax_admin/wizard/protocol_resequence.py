...
class ReSequenceWizard(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if (
            self.env.context["active_model"] == "account.move.bg.protocol"
            and "active_ids" in self.env.context
        ):
            active_protocol_ids = self.env["account.move.bg.protocol"].browse(
    ...
    def _compute_sequence_number_reset(self):
        ...
        """
        for record in self:
            record.sequence_number_reset = record.protocol_ids[
                0
        ...
        for record in self:
            if record.protocol_ids:
                record.first_name = min(
                    record.protocol_ids._origin.mapped(
                        lambda protocol: protocol.protocol_name or ""
                    )
                )
    ...
    def _compute_new_values(self):
        ...
        def _get_move_key(protocol_id):
            ...
            year_start, year_end = get_fiscal_year(
                protocol_id.date,
        ...
        for record in self.filtered("first_name"):
            protocols_by_period = defaultdict(
                lambda: record.env["account.move.bg.protocol"]
            )
            for protocol in (
                record.protocol_ids._origin
            ):  # Sort the moves by period depending on the sequence number reset
                protocols_by_period[_get_move_key(protocol)] += protocol

            seq_format, format_values = record.protocol_ids[
                0
            ]._get_sequence_format_param(record.first_name)
            format_values["year_end_length"] = len(
                str(date.today().year)
            )  # Set year_end_length based on the current year
            sequence_number_reset = record.protocol_ids[
                0
            ]._deduce_sequence_number_reset(record.first_name)
            ...
            for j, period_recs in enumerate(protocols_by_period.values()):
                ...
                year_start, year_end = period_recs[0]._get_sequence_date_range(
                    ...
                    seq_format.format(
                        **{
                            **format_values,
                            ...
                            + (
                                format_values["seq"]
                                if j == (len(protocols_by_period) - 1)
                                else 1
                            ),
                        }
                    )
                ...
                for protocol, new_name in zip(
                    period_recs.sorted(lambda m: (m.date, m.protocol_name or "", m.id)),
                    new_name_list,
                ):
    ...
    def resequence(self):
        ...
        if (
            self.protocol_ids.journal_id
            and self.protocol_ids.journal_id.restrict_mode_hash_table
        ):
        ...
        protocols_to_rename = self.env["account.move.bg.protocol"].browse(
        ...
        for protocol_id in self.protocol_ids:
            if str(protocol_id.id) in new_values:
                if self.ordering == "keep":
                    protocol_id.protocol_name = new_values[str(protocol_id.id)][
                        "new_by_name"
                    ]
                else:
                    protocol_id.protocol_name = new_values[str(protocol_id.id)][
