...
class AccountMove(models.Model):
    ...
    def _calculate_amount_for_different_purchases_taxes(self, options):
        ...
        total_amounts['vat_tax_40'] = sum(
            [inv.amount_tax for inv in invoices if inv.move_type in ['in_invoice', 'in_refund']])
    ...
    def _calculate_amount_for_different_sales_taxes(self, options):
        ...
        invoices = self._filter_invoices_type(options, self, ['out_invoice', 'out_refund'])
    ...
    def _get_document_type(self):
        ...
        return {'in_invoice': '01', 'out_invoice': '01', 'out_refund': '03', 'in_refund': '03'}.get(self.move_type,
                                                                                                    None)
    ...
    def _l10n_bg_get_amounts(self, company_currency=False):
        ...
        vat_taxes = tax_lines.filtered(lambda r: r.tax_line_id.tax_group_id.l10n_bg_vat_code)

        vat_taxable = self.env['account.move.line']
        for line in self.invoice_line_ids:
            if any(tax.tax_group_id.l10n_bg_vat_code and tax.tax_group_id.l10n_bg_vat_code not in ['0', '1', '2'] for
                   tax in line.tax_ids):
                vat_taxable |= line
        ...
        return {
            'vat_taxes_amount': sign * sum((tax_lines - vat_taxes).mapped(amount_field)),
            'vat_tag_11': sum([abs(line.balance) for line in
                               self.line_ids.filtered(lambda r: r.tax_tag_ids.id in account_tag_11_id)]),
            'vat_tag_21': sum([abs(line.balance) for line in
                               self.line_ids.filtered(lambda r: r.tax_tag_ids.id in account_tag_21_id)]),
        }
    ...
    def _get_vat(self):
        ...
        sign = -1 if self.move_type in ('out_refund', 'in_refund') and \
                     self.l10n_latam_document_type_id.code in self._get_l10n_bg_codes_used_for_inv_and_ref() else 1
        ...
        for tax_group in vat_taxable.mapped('tax_group_id'):
            base_imp = sum(self.invoice_line_ids.filtered(lambda x: x.tax_ids.filtered(
                lambda y: y.tax_group_id.l10n_bg_vat_code == tax_group.l10n_bg_vat_code)).mapped('price_subtotal'))
            imp = abs(sum(vat_taxable.filtered(
                lambda x: x.tax_group_id.l10n_bg_vat_code == tax_group.l10n_bg_vat_code).mapped('amount_currency')))
            res += [{'Id': tax_group.l10n_bg_vat_code,
                     'BaseImp': sign * base_imp,
                     'Importe': sign * imp}]

        # Report vat 0%
        vat_base_0 = sum(self.invoice_line_ids.filtered(
            lambda x: x.tax_ids.filtered(lambda y: y.tax_group_id.l10n_bg_vat_code == '0')).mapped('price_subtotal'))
        if vat_base_0:
