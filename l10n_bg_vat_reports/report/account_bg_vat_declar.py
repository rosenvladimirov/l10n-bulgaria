...
from odoo.addons.l10n_bg_vat_reports.report.l10n_bg_file_helper import l10n_bg_lang, l10n_bg_where, l10n_bg_odoo_compatible
...
class AccountBgVatInfoDeclar(models.Model):
    ...
    def _select(self):
        ...
        if self._context.get('report_options') and self._context['report_options'].get('lang'):
            lang_code = self._context['report_options']['lang']

        company_city = l10n_bg_lang(self.env, "company_partner.city", lang_code=lang_code)
        company_street = l10n_bg_lang(self.env, "company_partner.street", lang_code=lang_code)
        company_name = l10n_bg_lang(self.env, "company_partner.name", lang_modules="partner", lang_code=lang_code)
        represent_name = l10n_bg_lang(self.env, "represent_partner.name", lang_modules="partner", lang_code=lang_code)

        return f"""acc.company_id AS company_id,
        COALESCE(company_partner.vat, company_partner.l10n_bg_uic) AS company_vat,
        CONCAT ({company_city}, ', ', {company_street}) AS company_address,
        COALESCE(company_partner.vat, company_partner.l10n_bg_uic) AS info_tag_1,
        {company_name} AS info_tag_2,
        info_tag_3,
        CONCAT (represent_partner.l10n_bg_uic, ' ', {represent_name}) AS info_tag_4,
        acc.info_tag_5,
        acc.info_tag_6,
        acc.account_tag_10,
        acc.account_tag_11,
...
{'GROUP BY ' + self._group() or ''}"""
    ...
    def _select(self):
        ...
        SUM(accs.account_tag_11 + accs.account_tag_121 + accs.account_tag_122 + accs.account_tag_13 + accs.account_tag_15 + accs.account_tag_16 + accs.account_tag_18 + accs.account_tag_19) AS account_tag_10,
        ...
        COALESCE(SUM(accs.account_tag_21), 0.0) AS account_tag_21,
        ...
        0.0 AS account_tag_33,
        0.0 AS account_tag_43"""
...
LEFT JOIN (SELECT move_id, date, account_tag_30, account_tag_31, account_tag_41, account_tag_32, account_tag_42,
                  account_tag_43, account_tag_44, state FROM account_bg_calc_purchases_line AS acc{' WHERE ' + where_clause.replace('am.', 'acc.') if where_clause else ''}) AS accp
